...
def _auto_id():return ''.join(random.choice(_AUTO_ID_CHARS) for _ in six.moves.xrange(20))
...
def extractionFromBarcodesCollection(b_collection,path):
    ...
    for random_doc in b_collection:
        ...
        if flag1==True:#here we need to check all conditions using if-statements
            nowTime=time.time()
            nowTime=math.ceil(nowTime*1000)
            ts=str(nowTime)
...
for doc in DocumentsArray:
    ...
    if 'transferTo' in my_dict:
        ...
        for i in range(len(everythingArray)):
            ...
            if modifiedValue==False:
                ...
                if (priceArrayLengthIndb2==1):
                    db1.collection(u'users').document(idToTransferAtdb1).collection(u'barcode_inventory').document(everythingArray[i][2]).set
                    (
                        {
                            'barcodeName':everythingArray[i][1],
                            'barcodeNumber':everythingArray[i][2],
                            'barcodePrice':everythingArray[i][3][0]
                        }
                     )
                    ...
                    (
                        {
                            ...
                            'barcodeNumber':everythingArray[i][2],
                            'barcodePrice':everythingArray[i][3][0],
                            'barcodePrice2':everythingArray[i][3][1]
                        }
                    )
                    ...
                    (
                        {
                            ...
                            'barcodePrice':everythingArray[i][3][0],
                            'barcodePrice2':everythingArray[i][3][1],
                            'barcodePricePkt':everythingArray[i][3][2]
                        }
                    )
            elif modifiedValue==True:
                priceArrayLengthIndb2=everythingArray[i][4]
                if (priceArrayLengthIndb2==1):
                    db1.collection(u'users').document(idToTransferAtdb1).collection(u'barcode_inventory').document(everythingArray[i][2]).update
                    (
                        {
                            'barcodeName':everythingArray[i][1],
                            'barcodeNumber':everythingArray[i][2],
        ...
        reason1='transferTo field not found'
        pathVariable='/Kiranas/'+ids
        db2.collection(mainCollectionCreated).document(_auto_id()).set
        (
            {'path':pathVariable,
             'reason':reason1,
             'timestamp':ts,
             'transferRequestFor':str(ids)
             }
        )#autogenerateID(need to print path to find its type in python)
