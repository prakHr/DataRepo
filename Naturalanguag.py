...
nQ=[]
queries=[8906008970524,#E Vita Namkeen Chinese Frenky(30 gm),
8901123005700
]
import time
...
import webbrowser
#function takes queries of barcodes from nQ list and uses the search api to request google search sites(top 3)
#since only 1 request can be made per day reminder to use large len of nQ(1000s of barcodes to use this function)
def function2(nQ):
    for term in nQ:
        for j in search(term,tld="co.in",num=3,stop=1,pause=2):
            print(j)
        print('-------------------------')
        
#function takes queries of barcodes from nQ list and opens corresponding google/amazon page     
def function1(nQ):
    tabUrl="http://google.com/?#q="
    #tabUrl="https://www.amazon.in/s?url=search-alias%3Daps&field-keywords="
    for term in nQ:
        time.sleep(3)#so that doesn't consider us as robot
